...
const credentialPortrait = resolve(
  projectRoot,
  "public/media/profile/tajimerose-profile.webp",
);
...
const publicMediaBudgets = [
  {
    label: "Hero poster",
    path: resolve(projectRoot, "public/media/hero/tajimerose-hero.webp"),
    maxBytes: 200 * 1024,
  },
  {
    label: "Hero mobile poster",
    path: resolve(projectRoot, "public/media/hero/tajimerose-hero-mobile.webp"),
    maxBytes: 150 * 1024,
  },
] as const;

if (!existsSync(heroSource)) {
  errors.push("The owner-supplied hero source image is missing.");
} else {
...
if (errors.length > 0) {
  ...
  console.log(
    `Media validation passed. Hero images and credential portrait are within budget; ${managedMediaCount} candidate/published project asset(s) checked.`,
  );
}
