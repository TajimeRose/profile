...
## TajimeRose ownership update — 23 July 2026

This update supersedes the Wildan-specific identity, project map, hero video, and
contact details below. The portfolio now belongs to Wongsathon Witthayakhom
(TajimeRose), positioned as a Frontend Developer and computer science student.
The Ember Lab layout, accessibility requirements, and quality gates remain.

The current public routes are `/`, `/about`, `/work`, `/work/nongplatoo-ai`, and
`/work/dev-life`. The homepage Selected Work contains NongPlatoo.Ai as a ปวช.
project and DEV-LIFE as a Bangkok University project. The work index separates
the two education stages. The hero is an owner-supplied still image; no talking
video or third-party portrait is part of the current experience. This identity
and content replacement was explicitly requested by the owner on 23 July 2026.
...
For the TajimeRose portfolio, the current scene contains two panels:
NongPlatoo.Ai (`ปวช. Project`) and DEV-LIFE (`BU Project`). The same education
labels are used on the work index and case-study pages.
