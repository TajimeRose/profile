...
## TajimeRose media update — 23 July 2026

The current hero uses an owner-supplied responsive still image. Talking-video
autoplay, audio opt-in, offscreen video control, and video-failure states are no
longer active requirements. The Builder Pass, CSS interaction rules,
reduced-motion support, and normal-flow mobile behavior remain. Selected Work
currently contains NongPlatoo.Ai and DEV-LIFE, restoring the existing desktop
multi-panel transition while mobile and reduced-motion modes remain in normal
document flow.
