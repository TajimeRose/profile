...
## Current content baseline — 23 July 2026

The typed public content now represents Wongsathon Witthayakhom (TajimeRose) and
two portfolio records: NongPlatoo.Ai and DEV-LIFE. Static project generation
therefore produces `/work/nongplatoo-ai` and `/work/dev-life`. Each record carries
an explicit education stage so the interface can distinguish ปวช. work from
Bangkok University work. The hero is a server-rendered responsive still image
with no video client island. The credential QR encodes TajimeRose’s public
Instagram profile. Wildan-specific examples below are retained only as
historical architecture notes and do not describe current public data.
