            ...
            <Container>
              <div className="max-w-[46rem]">
                ...
                <p className="mt-5 text-lg leading-8 text-ink-600">
                  Interface captures from the working {project.title} project.
                </p>
              </div>
              <div className="mt-10 grid gap-7 lg:grid-cols-2">
                {project.media.slice(1, 3).map((media) => (
                  <figure
                    key={media.state === "placeholder" ? media.label : media.src}
                ...
                <div className="mt-20">
                  <div className="max-w-[46rem]">
                    <h2 className="font-display text-5xl font-[520] tracking-[-0.05em]">
                      {project.educationStage === "vocational"
                        ? "Competition moments"
                        : "More product screens"}
                    </h2>
                    <p className="mt-5 text-lg leading-8 text-ink-600">
                      {project.educationStage === "vocational"
                        ? "Photographs supplied by the project owner documenting the team and presentation environment."
                        : "Additional owner-supplied screens documenting the workspace, connected tools, and project settings."}
                    </p>
                  </div>
                  <div className="mt-10 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
                    {project.media.slice(3).map((media) => (
                      <figure
                        key={media.state === "placeholder" ? media.label : media.src}
