...
describe("validatePortfolioContent", () => {
  ...
  it("rejects duplicate slugs", () => {
    const duplicate = { ...projects[0], canonicalId: "project-duplicate" } as ProjectRecord;
    const result = validatePortfolioContent(
      [projects[0], duplicate],
      siteContent,
    );
  ...
  it("rejects project-specific blocked claims", () => {
    const invalid = {
      ...projects[0],
      cardCopy: "This production-ready assistant offers guaranteed accuracy.",
    } as ProjectRecord;
    const result = validatePortfolioContent(
      [invalid],
      siteContent,
  ...
  it("rejects blocked visitor-facing language in project links", () => {
    const invalid = {
      ...projects[0],
      links: projects[0].links.map((link, index) =>
