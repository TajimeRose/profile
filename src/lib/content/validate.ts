...
const approvedEvidenceIds: Record<string, readonly string[]> = {
  "nongplatoo-ai": ["nong-repo-01"],
  "dev-life": ["dev-repo-01"],
...
> = {
  "nongplatoo-ai": { status: "prototype", liveStatus: "offline" },
  "dev-life": { status: "active-build", liveStatus: "offline" },
...
export function validatePortfolioContent(
  ...
  if (records.length < 1) {
    errors.push("Expected at least 1 public project.");
  }
  ...
  for (const project of records) {
    ...
    const requiredText = {
      ...
      role: project.role,
      year: project.year,
      educationLabel: project.educationLabel,
